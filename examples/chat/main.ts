...
const reset = () => { epoch++; transport?.abort(); active = null; response.textContent = ''; events.textContent = 'No request yet.'; status.textContent = 'Ready'; ready(); };
...
form.addEventListener('submit', async event => {
  ...
  try {
    ...
    if (run === epoch && !controller.signal.aborted) status.textContent = `Connection error: ${error instanceof Error ? error.message : String(error)}`;
