...
export function ConfiguredRunner({selector}:{selector:React.ReactNode}){
 ...
 <section className="panel" aria-live="polite"><h2 data-testid="configured-verdict">{busy?'Running…':report?report.kind.toUpperCase():'Ready'}</h2><p>{report?.message}</p>{report?.assertions.map((a,i)=><p key={i}>{a.passed?'PASS':'FAIL'} · {a.description}</p>)}</section>
 <section className="panel"><h2>Observed events</h2><ol data-testid="configured-events">{events.map((event,i)=><li key={i}><code>{event.atMs} ms</code> · {event.kind} · {event.message}</li>)}</ol></section></>}
 </main>;
}
